...
sequenceDiagram
    ...
    C->>F: HTTP Request + Cookie / Authorization Header / oneTimeToken
    alt oneTimeToken 유효
        F->>A: findById(memberId)
        A-->>F: Member
...
- `/*/api/*/**` 는 인증 필요
- 단, 각 컨텍스트 configurer 와 `/sse/**` 같은 예외 경로가 공개 API 를 열어준다
...
### 1. SSE / WebSocket 경로는 `oneTimeToken` 도 지원한다

브라우저가 헤더/쿠키를 다루기 애매한 연결에서는 쿼리 파라미터 기반 일회용 토큰으로 인증한다.

### 2. accessToken 이 있으면 payload 기반으로 바로 인증한다

즉 매 요청마다 DB 를 반드시 치는 구조가 아닐 수 있다.

### 3. accessToken 이 없거나 무효면 apiKey 로 회원을 찾고 새 토큰을 발급한다
...
### 5. 헤더와 쿠키, 그리고 일부 경로의 `oneTimeToken` 까지 지원한다
...
대부분의 엔티티가 어떤 기반을 상속받는지 보면 프로젝트 철학이 더 잘 보인다.
...
- `equals` / `hashCode` 를 엔티티 식별자 규칙에 맞게 직접 정의한다.
...
그래서 2강 이후부터 엔티티를 읽을 때는 일단 `BaseTime` 기반인지 먼저 보고, 예외적으로 `BaseEntity` 만 상속하는 엔티티가 있는지도 함께 확인하면 된다.
...
> 프론트는 쿠키를 포함해서 API 를 호출하고, 백엔드 필터는 쿠키/헤더 토큰과 일부 실시간 경로의 `oneTimeToken` 을 읽어 현재 사용자를 복원한다.
...
classDiagram
    ...
    CustomAuthenticationFilter --> ActorFacade
    CustomAuthenticationFilter --> OneTimeTokenService
...
- 쿠키 인증, 헤더 인증, `oneTimeToken` 인증은 각각 어디서 쓰이는가?
...
[HTTP 요청]
    ↓
[CustomAuthenticationFilter]
  - 쿠키/헤더에서 apiKey, accessToken 추출
  - SSE/WS 는 oneTimeToken 도 처리 가능
...
게시글을 작성하면 현재 `PostDto` 기준으로는 대략 이런 응답이 온다.
...
```json
{
  ...
  "data": {
    ...
    "authorId": 2,
    "authorName": "관리자",
    "authorProfileImgUrl": "http://localhost:8080/member/api/v1/members/2/redirectToProfileImg",
    "title": "첫 번째 글",
    "published": true,
    "listed": true,
    "likesCount": 0,
...
**① `oneTimeToken` 이 있으면**: SSE / WebSocket 경로에서 먼저 검증한다. 성공하면 바로 인증하고 끝난다.
...
이 프로젝트는 권한을 DB 컬럼에서 읽지 않는다. 현재 관리자 여부는 `Member.isAdmin` 이 `username in setOf("system", "admin")` 인지로 판단하고,
`MemberHasSecurity` 가 그 결과를 `ROLE_ADMIN` 으로 변환한다.

그래서 JWT에 `username` 만 있으면 권한 계산에 필요한 정보도 함께 갖고 있는 셈이다. 이 구조에서는 별도 권한 클레임을 넣지 않아도 되고, 인증 후 권한을 위해 DB를 다시 볼 필요도 없다.
...
@PostMapping("/login")
@Transactional(readOnly = true)
fun login(@RequestBody @Valid reqBody: MemberLoginRequest): RsData<MemberLoginResBody> {
...
`maxAge` 는 값이 있으면 1년, 비우면 0으로 설정된다. 그래서 같은 메서드로 저장과 삭제를 모두 처리할 수 있다.
...
`rq.deleteCookie(...)` 는 내부적으로 `setCookie(name, null)` 을 호출해서 쿠키의 `maxAge` 를 0으로 만든다. 서버에는 세션도 없고 블랙리스트도 없다. 순수하게 클라이언트 쿠키만 지운다.
...
sequenceDiagram
    ...
    Note over Filter: 로그인 경로 → 인증 로직 스킵
    Filter->>CT: 요청 전달
...
data class OAuth2State(
    ...
    companion object {
        fun decode(encoded: String): OAuth2State {
            ...
            val parts = decoded.split("#", ignoreCase = false, limit = 2)
...
15강부터는 공통 기반 파트다. 엔티티들이 공통으로 활용하는 `BaseEntity` 와 `BaseTime` 을 본다.
...
> 엔티티들이 공통으로 활용하는 두 기반 클래스를 이해하고, 어디서 갈라지는지까지 파악하기.
...
대부분의 도메인 엔티티는 `BaseTime` 을 상속하고, `BaseTime` 은 다시 `BaseEntity` 를 상속한다. 다만 로그성 엔티티처럼 `BaseEntity` 만 상속하는 예외도 있다.

---

## 2. `BaseEntity`: 식별자와 엔티티 공통 유틸을 담당한다
...
- `attrCache` 로 같은 엔티티 안에서 속성 레코드 조회를 캐시한다.
- `equals` / `hashCode` 를 직접 정의해서 "저장된 엔티티는 id 기준", "저장 전 엔티티(id=0)는 서로 같지 않다"는 규칙을 강제한다.

또 `Hibernate.getClass(...)` 를 써서 프록시 클래스와 실제 엔티티 클래스를 같은 타입으로 취급한다. 단순 `javaClass` 비교보다 Hibernate 환경에 안전하다.
...
`@CreatedDate` 는 저장 시 자동으로 채운다. `@LastModifiedDate` 는 수정 시 자동으로 갱신한다. `BaseTime` 을 상속한 엔티티들은 `createdAt`, `modifiedAt` 을 자동으로
관리한다.
...
문자열 `"CREATED_AT"` 이 자동으로 enum으로 변환되고, enum이 `sortBy` 계산 프로퍼티로 `Sort` 객체를 제공한다. `Pageable` 에 그대로 넘긴다.
...
`ActorFacade` 와 `AuthTokenService` 가 `app/shared/` 에 있다. `ActorFacade` 는 인증 필터(`CustomAuthenticationFilter`) 와 OAuth2 성공 핸들러에서 직접 쓰이고,
`AuthTokenService` 는 그 뒤에서 토큰 생성/해석 책임을 맡는다.
...
기본 생성자는 `apiKey` 까지 받지만, 외부에서는 이 보조 생성자를 주로 쓴다. 이 생성자에서 `MemberPolicy.genApiKey()` 를 호출해 초기 `apiKey` 를 채운다.
...
`MemberAware` 는 mixin 들이 공통으로 기대하는 최소 계약이다. `member` 참조뿐 아니라 `id`, `name` 도 함께 노출해서, 구현체가 `Member` 라는 사실을 전제로 공통 로직을 쓸 수 있게 한다.
...
| `ApiV1MemberController`    | `/member/api/v1/members`     | 회원가입/프로필 이미지 redirect/보안 팁 |
...
class MemberActionLogEventListener(
    ...
    // 같은 패턴으로 PostDeletedEvent, PostCommentWrittenEvent, PostCommentModifiedEvent,
    // PostCommentDeletedEvent, PostLikedEvent, PostUnlikedEvent 등도 등록
...
class MemberActionLog(
    val type: String,          // 이벤트 타입 (e.g. "PostWrittenEvent")
...
`MockMvc` 는 서블릿 레이어를 직접 통과시켜서 HTTP 없이 처리한다. 같은 JVM 프로세스 안에서 동작하므로 네트워크 오버헤드가 없다.

하지만 이 선택의 핵심은 단순 성능 최적화만이 아니다. 같은 애플리케이션 안에 있더라도 `post` 가 `member` 의 구현을 직접 부르지 않고 **내부 API 경계**를 통해 접근하게 해서,
바운디드 컨텍스트 간 코드 레벨 결합을 낮추려는 의도도 있다. `build.gradle.kts` 에 `implementation("org.springframework:spring-test")` 가 있는 이유가
바로 이것이다. 테스트 의존성이 아니라 프로덕션 의존성으로 추가돼 있다.
...
`canRead` / `checkActorCanRead` 는 조회 권한 전용이다. `published` 가 `false` 이면 작성자와 관리자만 조회 가능하고, 아니면 누구나 읽을 수 있다.
...
댓글 조회 권한은 별도 mixin 이 아니라, 컨트롤러에서 먼저 `post.checkActorCanRead(...)` 로 부모 글 권한을 확인하는 방식이다.
...
`PostMember` 는 `post` 패키지에 있지만 `MemberAware` 를 상속한 Kotlin 인터페이스다. `Member` 클래스가 이 인터페이스를 직접 구현하고, 실제 동작은
`PostMember.kt` 의 디폴트 멤버가 제공한다.

그래서 `PostFacade.write` 의 `author.incrementPostsCount()` 호출은 `Member` 타입에서 바로 성립한다. `MemberProxy` 역시 `Member` 를 상속하므로 같은
`PostMember` 기본 구현을 그대로 사용한다.
...
`aggregateType` 은 여기서 `postDto::class.simpleName!!` 이므로 `"PostDto"` 다. `aggregateId` 는 `postDto.id` 다. 이 두 필드는 `Task` 엔티티에도 저장되어,
어떤 DTO 스냅샷을 기준으로 만들어진 이벤트인지 빠르게 파악할 수 있게 해준다.

`postDto` 는 `@JsonIgnore` 이지만 `@JsonGetter` 로 `forEventLog()` 결과를 반환한다. 현재 `PostDto.forEventLog()` 는 `title` 만 비우고 나머지 메타데이터는 유지한다.
...
- `IntegrationTestBase` 를 상속하는 `@SpringBootTest` 기반 통합 테스트
- 스프링 컨텍스트 없이 도는 순수 도메인 테스트
...
test/
  ...
  member/
    ...
    domain/shared/memberMixin/MemberSecurityMixinTest.kt
  ...
  post/
    app/PostSseServiceTest.kt
    app/PostStompServiceTest.kt
    config/PostWebSocketSecurityConfigTest.kt
    domain/PostTest.kt
    domain/PostPolicyTest.kt
    domain/PostCommentPolicyTest.kt
    in/ApiV1PostControllerTest.kt
    in/ApiV1PostCommentControllerTest.kt
  global/
    sse/app/SseServiceTest.kt
```
...
`IntegrationTestBase` 계열 테스트는 `@Transactional` 이다. 테스트가 끝나면 롤백된다. DB 상태가 테스트 간에 오염되지 않는다.

반면 `MemberTest`, `PostTest`, `PostPolicyTest` 같은 순수 도메인 테스트는 스프링 컨텍스트를 띄우지 않고 JUnit 만으로 실행된다.

`@ActiveProfiles("test")` 는 통합 테스트에서 테스트용 datasource 설정을 활성화한다. 개발용 DB 와 분리된다.
...
통합 테스트 계열은 `@SpringBootTest` 를 공통으로 쓰고, `@DataJpaTest`, `@WebMvcTest` 같은 슬라이스 테스트는 두지 않는다. 컨텍스트 로딩이 무겁지만, 실제 환경에 가까운 테스트가 가능하다.
...
`PostMember` 는 `post` 패키지에 있지만 `MemberAware` 를 상속한 인터페이스이고, `member` 패키지의 `Member` 클래스가 이를 직접 구현한다. 덕분에 `member` 쪽은
`Post` 엔티티 자체를 직접 들고 있지 않아도 회원의 글/댓글 집계 로직을 붙일 수 있다.
...
class SpringDocConfig {
    ...
    fun groupController(): GroupedOpenApi =
        GroupedOpenApi.builder().group("controller").pathsToExclude("/*/api/v1/**").build()
}
```
...
이 파트는 백엔드와 직접 맞닿는 프론트 소비 지점만 다룬다. 프론트 자체 구조와 실행법은 [front/README.md](C:/Users/jangk/IdeaProjects/slog_2026_03/front/README.md)를 기준 문서로 본다.
...
- `@stomp/stompjs` + `sockjs-client`: STOMP WebSocket
- `next-themes`: 테마 전환
- `sonner`: Toast 알림
- `@toast-ui/react-editor`: 에디터/뷰어
...
프론트에는 STOMP + SockJS 클라이언트가 있고, 별도로 SSE 클라이언트도 함께 쓴다. 글 수정 실시간 반영은 STOMP, 새 글 알림은 SSE가 담당한다.
...
여기서의 `subscribe("/topic/posts/new", handler)` 는 STOMP 구독 헬퍼다. 연결 여부에 관계없이 `Promise<ManagedSubscription>` 을 받는다. 반환된
`subscription.unsubscribe()` 로 구독 해제 시 `activeSubscriptions` 에서도 제거된다.

---

## 3. useNewPostNotification (SSE)
...
export function useNewPostNotification(onNewPost?: (post: PostNotification) => void) {
  ...
  }, [isPending, isLogin, onNewPost]);
}
```

`posts-new` SSE 채널을 구독한다. 로그인 상태면 내부적으로 `/member/api/v1/auth/oneTimeToken` 으로 일회용 토큰을 받아 붙이고, 비로그인 상태면 공개 채널로 바로 연결한다. 언마운트 시 구독을 해제한다.
...
- JPA `@DynamicUpdate` + `@SQLRestriction` + soft delete
...
현재는 기본 계정(`system`, `admin`, `holding`, `user1`, `user2`, `user3`) 외에도 `custom.notProdMembers` 설정에 들어 있는 계정들을 추가로 생성한다.
...
컨텍스트 초기화 완료 시점에 Spring 빈을 순회하면서 `@TaskHandler` 메서드를 찾는다. 현재 구현은 singleton 빈만 대상으로 하고, 파라미터가 정확히 1개이면서 `TaskPayload`
하위 타입일 때만 등록한다. 컴파일 없이 핸들러를 추가할 수 있다 — 메서드에 `@TaskHandler` 를 붙이고 payload 클래스에 `@Task` 를 달면 된다.
...
enum 자체는 단순 값이고, `sortBy` / `property` / `isAsc` 를 계산 프로퍼티로 제공한다. 컨트롤러에서:
...
2. **`username` 기반 `AuthenticationManager` 경로** — Spring Security 가 `UserDetailsService` 가 필요할 때

`CustomAuthenticationFilter` 는 이 서비스를 직접 쓰지 않는다. 필터는 토큰/API 키 기반이어서 `username` → `UserDetails` 조회 경로를 타지 않는다.

현재 `ApiV1AuthController.login` 은 `AuthenticationManager` 를 거치지 않고, `memberFacade.findByUsername(...)` 와 `memberFacade.checkPassword(...)` 를 직접 호출한다.
즉 이 서비스의 주된 확인 가능한 사용 지점은 `@WithUserDetails` 테스트 쪽이다.
...
**단일 앱이지만 멀티 인스턴스를 고려한 구조**
현재 배포 단위는 모노레포 + 단일 Spring Boot 앱이지만, 설계는 "어차피 한 프로세스니까 서로 직접 부르자" 쪽이 아니다. 바운디드 컨텍스트 사이의 코드 레벨 직접 접근을 최대한 막고,
정말 예외적인 경우(`Member`, `PostMember`)를 제외하면 경계 너머 기능은 내부 API를 통해 접근한다.

`post` 쪽에서 `member` 기능이 필요할 때 `MemberApiClient` + `InternalRestClient` 를 쓰는 이유도 여기 있다. 같은 앱 안에서 동작하더라도 **API 경계는 유지**하고,
나중에 멀티 인스턴스나 서비스 분리 쪽으로 가더라도 구조를 덜 흔들리게 하려는 선택이다.

**JPA 중심 개발**
QueryDSL 로 동적 쿼리를 처리하고, `SEQUENCE` 기반 식별자 생성과 `@DynamicUpdate` 로 영속 계층 동작을 제어한다.

**도메인 중심 설계**
비즈니스 규칙을 엔티티와 Mixin 에 넣는다. Facade 는 얇다. 테스트에서 Facade 를 직접 호출해서 비즈니스 로직을 검증할 수 있다.

**애그리거트 경계는 Repository 개수로 정하지 않음**
`PostComment` 에 `Repository` 가 있더라도, 현재 의도는 댓글을 글에 속한 것으로 보는 것이다. 그래서 `post.findCommentById()` 처럼 부모 `Post` 를 통해 자식을 조회하고,
댓글의 읽기 권한도 먼저 `post.checkActorCanRead(...)` 로 확인한다.

즉 이 프로젝트는 "Repository가 따로 있으면 곧바로 별도 애그리거트"라고 보지 않는다. 애그리거트 경계는 **생애주기, 일관성 경계, 트랜잭션 경계**로 정하며, 현재는 댓글을 Post에 강하게
묶어 관리하는 쪽을 선택했다.
...
**완전히 독립 배포되는 마이크로서비스**
바운디드 컨텍스트 경계는 강하게 의식하지만, 현재는 여전히 하나의 Spring Boot 앱으로 배포한다. 즉 네트워크 수준의 완전 분리보다, **코드 레벨 경계 유지 + 단일 실행 단위** 쪽을 택했다.

**교과서적 의미의 완전한 헥사고날 아키텍처**
`in/` / `out/` 구조와 내부 API 호출로 경계를 의식하지만, 모든 유스케이스를 엄격한 포트/어댑터 인터페이스로 감싸지는 않는다. 실용성과 학습 난이도를 함께 고려한 선택이다.

**CQRS**
읽기/쓰기 모델을 분리하지 않는다. 동일한 JPA 엔티티가 읽기와 쓰기 모두에 쓰인다.

**테스트 분리**
슬라이스 테스트(`@DataJpaTest`, `@WebMvcTest`)는 두지 않고, 통합 테스트는 `@SpringBootTest` 중심으로 가져간다. 빠른 테스트 피드백보다 실환경 근접성을 택했다.
